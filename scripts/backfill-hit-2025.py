#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""补哈尔滨工业大学**本部** 2025 专业级录取线(官方招生网,合规)。

数据源:zsb.hit.edu.cn 的 POST /information/score-list(一次性 CSRF token,逐省抓;见抓取 agent)。
官方只给最高/平均/最低分,**无位次** → min_rank 用本省 2025 一分一段按最低分换算(与华科同法、
与引擎给考生算位次同口径;换算精度已由中科大官方位次交叉校验 0 误差证明)。

**只补本部(uni_name=哈尔滨工业大学)**:威海/深圳校区库里已 25-28 省(且库内半角/全角命名+uni_id
混乱,uni_id 225 被全角威海/深圳误用),不在此动以免加重脏数据;本部原仅 11 省 → 31 省。
科类:源用「物理+化学」这类选科组合写法 → 归一到 物理类(并据此填 sel_req);「物理」「历史」「理科」
「文科」照映;「艺术类」跳过(无标准一分一段)。仅幂等 source='hit-2025-official',不动已有 11 省。

用法: python3 backfill-hit-2025.py [--commit] [--json /tmp/hit_2025.json] [--db ...]
"""
import json, sqlite3, argparse, statistics
from collections import Counter, defaultdict

UNI = "哈尔滨工业大学"; YEAR = 2025; SRC = "hit-2025-official"; UNI_ID = 225; CAMPUS = "本部"
...
def map_subject(subs, kl):
    """哈工大科类(物理+化学 | 物理 | 历史 | 理科 | 文科 | 艺术类)→ (rank科类, al.subject, al.subject_std)。"""
    if "艺术" in kl: return None, None, None              # 艺术无标准一分一段,跳过
...
def sel_of(kl):
    if "化学" in kl: return "物理、化学"
    if "物理" in kl: return "物理"
    if "历史" in kl: return "历史"
    return None

def is_clean(major):
    """只留普通类主线(与华科/中科大同口径):剔除 解析垃圾(『录取分数』列标签/汇总行)、
    中外合作办学(特殊高收费项目、分数低)、艺术类(无标准一分一段)。"""
    m = (major or "").strip()
...
def main():
    ...
    ap.add_argument("--json", default="/tmp/hit_2025.json")
    ap.add_argument("--db", default="gaokao-data/gaokao.db")
    ap.add_argument("--commit", action="store_true")
    args = ap.parse_args()
    c = sqlite3.connect(args.db)
    recs = [r for r in json.load(open(args.json, encoding="utf-8")) if r.get("campus") == CAMPUS]
    print(f"解析:哈工大**{CAMPUS}** 2025 专业级 {len(recs)} 条,{len(set(r['province'] for r in recs))} 省 "
          f"(全文件 {CAMPUS}/威海/深圳 三校区;本脚本只补本部)")
    ...
    print(f"库里本部已有 2025 专业级省({len(have)},跳过): {'、'.join(sorted(have))}")

    # ── 校验转换器:用库里本部已有省的真实 (min_score,min_rank) 验(哈工大源无位次,靠这个+单调性)──
    ...
    for r in recs:
        ...
        out.append({"prov": prov, "asub": asub, "astd": astd, "major": r["speciality"] if "speciality" in r else r["major"],
                    "ms": r["min_score"], "mr": mr, "avs": r.get("avg_score"),
                    "avr": rank_of(c, prov, rsub, r.get("avg_score")), "mxs": r.get("max_score"),
                    "sel": sel_of(r["subject"]), "batch": batch_for(c, prov, astd)})
    ...
    cur.executemany("""INSERT INTO admission_lines
        ...
        [(UNI_ID, UNI, None, o["prov"], YEAR, o["batch"], o["asub"], None, o["major"], None,
          o["sel"], o["ms"], o["mr"], o["avs"], o["avr"], o["mxs"], None, None, SRC, o["astd"]) for o in out])
    c.commit()
    ...
    print(f"\n=== 已写库:删旧 {deleted}、插新 {len(out)}。哈工大本部 2025 专业级现 {after[0]} 条 / {after[1]} 省 ===")
